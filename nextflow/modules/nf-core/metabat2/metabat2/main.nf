process METABAT2_METABAT2 {
    ...
    tuple val("${task.process}"), val('metabat2'), eval('metabat2 --help 2>&1 | sed -n "2s/.*:\\([0-9]*\\.[0-9]*\\).*/\\1/p"'), emit: versions_metabat2, topic: version
    ...
    find . -maxdepth 1 -name "*.fa" -type f | xargs -t -n 1 bgzip -@ ${task.cpus}
    """
    ...
    # FIXED: Added stub generation for versions.yml
